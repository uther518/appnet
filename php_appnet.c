...
/* Remove comments and fill if you need to have entries in php.ini
 PHP_INI_BEGIN()
 STD_PHP_INI_ENTRY("appnet.global_value",      "42", PHP_INI_ALL,
 OnUpdateLong, global_value, zend_appnet_globals, appnet_globals)
 STD_PHP_INI_ENTRY("appnet.global_string", "foobar", PHP_INI_ALL,
 OnUpdateString, global_string, zend_appnet_globals, appnet_globals)
 PHP_INI_END()
 */
/* }}} */
...
PHP_FUNCTION( confirm_appnet_compiled )
{
	...
	strg = strpprintf( 0 , "Congratulations! You have successfully modified "
			"ext/%.78s/config.m4. Module %.78s is now compiled into "
			"PHP." ,
			"appnet" , arg );
...
#define REGISTER_APPNET_CLASS_CONST_STRING(const_name, value)                  \
  zend_declare_class_constant_stringl(AppnetServer, const_name,                \
	...
	REGISTER_STRING_CONSTANT( "APPNET_OPT_REACTOR_NUM" , OPT_REACTOR_NUM ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_OPT_MAX_CONNECTION" , OPT_MAX_CONNECTION ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_OPT_PROTO_TYPE" , OPT_PROTOCOL_TYPE ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_OPT_DAEMON" , OPT_DAEMON ,
			CONST_CS | CONST_PERSISTENT );
	...
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_CONNECT" , APPNET_EVENT_CONNECT ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_RECV" , APPNET_EVENT_RECV ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_CLOSE" , APPNET_EVENT_CLOSE ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_START" , APPNET_EVENT_START ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_FINAL" , APPNET_EVENT_FINAL ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_TIMER" , APPNET_EVENT_TIMER ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_TASK" , APPNET_EVENT_TASK ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_EVENT_TASK_CB" , APPNET_EVENT_TASK_CB ,
			CONST_CS | CONST_PERSISTENT );
	
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_TCP_ONLY" , PROTOCOL_TYPE_TCP_ONLY ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_HTTP_ONLY" , PROTOCOL_TYPE_HTTP_ONLY ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_WEBSOCKET_ONLY" ,
			PROTOCOL_TYPE_WEBSOCKET_ONLY ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_HTTP_MIX" , PROTOCOL_TYPE_HTTP_MIX ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_WEBSOCKET_MIX" ,
			PROTOCOL_TYPE_WEBSOCKET_MIX ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_LONG_CONSTANT( "APPNET_PROTO_MIX" , PROTOCOL_TYPE_WEBSOCKET_MIX ,
			CONST_CS | CONST_PERSISTENT );
	
	REGISTER_STRING_CONSTANT( "APPNET_HTTP_KEEP_ALIVE" , APPNET_HTTP_KEEP_ALIVE ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_HTTP_DOCS_ROOT" , APPNET_HTTP_DOCS_ROOT ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_HTTP_UPLOAD_DIR" , APPNET_HTTP_UPLOAD_DIR ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_HTTP_404_PAGE" , APPNET_HTTP_404_PAGE ,
			CONST_CS | CONST_PERSISTENT );
	REGISTER_STRING_CONSTANT( "APPNET_HTTP_50X_PAGE" , APPNET_HTTP_50X_PAGE ,
			CONST_CS | CONST_PERSISTENT );
	
	// zend_declare_property_null(appTcpServer, "serv_ip", strlen("serv_ip"),
