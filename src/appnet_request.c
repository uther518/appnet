...
int readingSingleLine( httpHeader *header, const char *org, int len )
{
    ...
    if (upgrade == 1)
    {
        if (memcmp( value, "websocket", value_len ) == 0 ||  memcmp( value, "Websocket", value_len ) == 0)
        {
...
void parsePostRequest( httpHeader *header, sds buffer, int len )
{
    ...
    {
        appendHttpData( header->connfd, buffer, header->buffer_pos, buffer + header->buffer_pos, header->content_length,
...
int wesocketRequestRarse( int connfd, sds buffer, int len, httpHeader *header, handshake *hs )
{
    ...
    else
    {
        ...
        {
            /* make sure recv data is complete, then create task.. */
            appendWorkerData( connfd, recv_data, recv_len, PIPE_EVENT_MESSAGE, "parseWebsocket" );
        }
    }
    ...
    if (hs->state == WS_STATE_OPENING)
    {
        ...
        if (hs->frame_type == WS_OPENING_FRAME)
        {
            ...
            servG->connlist[connfd].send_buffer = sdscatlen( servG->connlist[connfd].send_buffer, (char *) out_buffer, frameSize );
        }
...
void putUploadFile( int connfd, char *filename, char *data, int len, char *destfile )
{
...
void parseMultipartForm( httpHeader *header, sds buffer, int len )
{
    ...
    while (1)
    {
        ...
        if (strlen( filename ))
        {
            printf( "Vlen=%d \n", vlen );
            char destfile[255] = {0};
            putUploadFile( header->connfd, filename, crlf + strlen( AE_HEADER_END ), vlen, &destfile );
