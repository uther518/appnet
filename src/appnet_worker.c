...
int createResponse( int connfd, char *buff, int len, char proto_type,sds response )
{
    if (proto_type == LISTEN_TYPE_HTTP )
    {
...
int sendMessageToReactor( int connfd, char *buff, int len )
{
    ...
    if (len >= 0)
    {
        /* if proto is http or websocket, need compose response data ,else direct
         send origin data */
        if (servG->worker->proto != LISTEN_TYPE_TCP )
        {
...
void readWorkerBodyFromPipe( int pipe_fd, appnetPipeData data )
{
    ...
    if (data.proto == LISTEN_TYPE_HTTP )
    {
...
void runWorkerProcess( int pidx )
{
    ...
    /* listen pipe event and expect reactor message come in; */
    for (thid = 0; thid < servG->reactor_num; thid++)
    {
        index = pidx * servG->reactor_num + thid;
        res = aeCreateFileEvent( worker->el, servG->worker_pipes[index].pipefd[1],AE_READABLE, onWorkerPipeReadable, NULL );
