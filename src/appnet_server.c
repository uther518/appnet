...
{
    return ( connfd % servG->worker_num ) * servG->reactor_num + connfd % servG->reactor_num;
}
...
void onClientWritable( aeEventLoop *el, int connfd, void *privdata, int mask )
{
    ...
    nwritten = write( connfd, servG->connlist[connfd].send_buffer, sdslen( servG->connlist[connfd].send_buffer ) );
    if (nwritten <= 0)
    {
...
void appendWorkerData( int connfd, char *buffer, int len, int event_type,  char *from )
{
    ...
    if (servG->connlist[connfd].proto_type == LISTEN_TYPE_TCP)
    {
...
int parseRequestMessage( int connfd, sds buffer, int len )
{
    ...
    if ( servG->connlist[connfd].proto_type == LISTEN_TYPE_TCP )
    {
        appendWorkerData( connfd, buffer, len, PIPE_EVENT_MESSAGE, "PROTOCOL_TYPE_TCP" );
        return BREAK_RECV;
    }
    else if( servG->connlist[connfd].proto_type == LISTEN_TYPE_HTTP )
    {
        ret = httpRequestParse( connfd, buffer, sdslen( buffer ) );
    }
    else if( servG->connlist[connfd].proto_type == LISTEN_TYPE_WS )
    {
        if( servG->connlist[connfd].wshs == 0  )
        {
            ret = httpRequestParse( connfd, buffer, sdslen( buffer ) );
        }
        else
        {
            int thid = servG->connlist[connfd].reactor_id;
            ret = wesocketRequestRarse( connfd, buffer, len,servG->reactor_threads[thid].hh,servG->reactor_threads[thid].hs );
        }
    }
    else
    {
...
void onClientReadable( aeEventLoop *el, int connfd, void *privdata, int mask )
{
    ...
    while (1)
    {
        ...
        {
            /* Append to client recv_buffer */
            servG->connlist[connfd].recv_buffer = sdscatlen( servG->connlist[connfd].recv_buffer, &buffer, nread );
            /* Protocal Parse */
            int ret = parseRequestMessage( connfd, servG->connlist[connfd].recv_buffer, sdslen( servG->connlist[connfd].recv_buffer ) );
            if (ret == BREAK_RECV)
            {
...
int setPipeWritable( aeEventLoop *el, void *privdata, int connfd )
{
    int worker_id = servG->connlist[connfd].worker_id;
    /* pipe index */
    int index = worker_id * servG->reactor_num + servG->connlist[connfd].reactor_id;
    if (sdslen( servG->workers[worker_id].send_buffer ) == 0)
    {
        int ret = aeCreateFileEvent( el, servG->worker_pipes[index].pipefd[0], AE_WRITABLE, onMasterPipeWritable, worker_id );
        if (ret == AE_ERR)
        {
...
void acceptCommonHandler( appnetServer *serv, int listenfd, int connfd, char *client_ip, int client_port )
{
    ...
    int listen_type;
    for( listen_type=0; listen_type<LISTEN_TYPE_MAX; listen_type++ )
    {
        if( listenfd == serv->listen_fds[listen_type] )
        {
            break;
        }
    }
    ...
    serv->connlist[connfd].wshs = 0;
    serv->connlist[connfd].send_buffer = sdsempty();
    serv->connlist[connfd].recv_buffer = sdsempty();
    serv->connlist[connfd].proto_type = listen_type;
    serv->connlist[connfd].listen_port = serv->ports[listen_type];
    serv->connlist[connfd].listen_fd = listenfd;
    ...
    if (connfd != -1)
    {
        ...
        if (aeCreateFileEvent( el, connfd, AE_READABLE, onClientReadable, &connfd ) == AE_ERR)
        {
...
void onAcceptEvent( aeEventLoop *el, int fd, void *privdata, int mask )
{
    if ( /*servG->listenfd == fd */  fd > 0 )
    {
        ...
        while (max--)
        {
            ...
            acceptCommonHandler( servG, fd, connfd, client_ip, client_port );
        }
    }
    else
    {
...
void runMainReactor( appnetServer *serv )
{
    ...
    {
        if( serv->listen_fds[i] > 0 )
        {
            res = aeCreateFileEvent( serv->main_reactor->event_loop, serv->listen_fds[i], AE_READABLE, onAcceptEvent, NULL );
            printf( "Master Listen Port=%d,Socketfd=%d \n", serv->ports[i], serv->listen_fds[i]);
        }
    }
    ...
    while (1)
    {
        ...
        if (nread > 0)
        {
            ...
            else
            {
                ...
                if (sdslen( servG->workers[worker_id].send_buffer ) == 0)
                {
                    int result = aeCreateFileEvent( el, servG->worker_pipes[index].pipefd[0],
...
void initServer( appnetServer *serv )
{
    ...
    serv->reactor_threads = zmalloc( serv->reactor_num * sizeof(appnetReactorThread) );
    serv->workers = zmalloc( ( serv->worker_num + serv->task_worker_num ) * sizeof(appnetWorkerProcess) );
    serv->worker_pipes = zmalloc( ( serv->worker_num + serv->task_worker_num ) * serv->reactor_num * sizeof(appnetWorkerPipes) );
...
void listenPorts( appnetServer* serv )
{
    ...
    for( i = 0; i < LISTEN_TYPE_MAX ; i++ )
    {
        ...
        if( serv->ports[i] > 0 )
        {
            listenToPort( serv->listen_ip, serv->ports[i], sockfd, &sock_count );
            serv->listen_fds[i] = sockfd[0];
        }
    }
}
